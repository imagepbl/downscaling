...
def process_urban_classification_data(project_dir: Path, log: logging.Logger=local_log) -> None:
    ...
    2. Create raster file for base year polygons
    '''
    ...
    # 2. Create raster file for base year polygons
    # TO DO --> needs a xarray dataset as input, not sure if this is the right place to do it, but for now just log the message
    log.info(f"\n{PRINT_COLORS["green"]}Creating raster file for base year polygons...{PRINT_COLORS["end"]}")
...
def create_urban_id_raster(gdf: gpd.GeoDataFrame, ds_ref: xr.Dataset, cache_path: Path | None = None,
                           use_saved: bool = True) -> np.ndarray:
    '''
    ...
    if use_saved and cache_path is not None and Path(cache_path).exists():
        ...
        print(f"{PRINT_COLORS["yellow"]}Cached id raster at {cache_path} does not exist; creating it now.{PRINT_COLORS["end"]}")

    if gdf.crs != ds_ref.rio.crs:
        gdf = gdf.to_crs(ds_ref.rio.crs)
    id_dtype = "uint16" if len(gdf) <= np.iinfo("uint16").max else "int32"
    ids = rasterio.features.rasterize(((geom, i) for i, geom in enumerate(gdf.geometry, start=1)), out_shape=out_shape,
