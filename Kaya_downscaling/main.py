...
from tools.convert_GIS import GADM_vector_to_raster

import downscaling.plot_maps as plot_maps #plot_model_region_country_raster, plot_Mercator_projection_update, plot_factors_GDP_POP, plot_IPAT_summary
...
def create_GADM_region_raster(model:str="IMAGE", resolution_minutes:float=0.5, plot=False):
    DIR = Path(__file__).parent
    project_dir = str(DIR)
    dir_GADM = f"{project_dir}/data/processed/GADM"
    ...
    if not Path(iso_GADM_raster_file).exists():
        ...
        raster_file, iso_to_id, id_to_iso = GADM_vector_to_raster(project_dir, dir_GADM, resolution_degrees = resolution_minutes/60, plot=False)
    ...
    if model == "IMAGE":
        df_model_coutry_to_region = pd.read_csv(f"{project_dir}/data/input/models/IMAGE/country_to_regions.csv", sep=";") # ISO3
        ...
        df_region_numbers = pd.read_csv(f"{project_dir}/data/input/models/IMAGE/image_region_numbers.csv", sep=",")
    ...
    ds_GADM_raster.to_netcdf(f"{dir_GADM}/IMAGE_GADM_regions_raster_{res_min_file_end}_arcmin.nc", mode="w", engine="h5netcdf")
...
def process_datasets(SSP_base="SSP2", copy:bool=False):
    # settings
    #SSP_base="SSP2"
    data_sources = [
...
def downscale_SE_data(variable_SE: str, scenario: str, model: str = "IMAGE", align_resolution: bool = True, profile: str = "default"):
    ...
    #project_dir = Path(__file__).parent.resolve()
    project_dir = Path(f"Z:/cold_data_storage/users/roelfsemam/surdrive_UU/NSA/Downscaling_share/Results")
    if align_resolution:
    ...
    # ----------------------------------------------------------------------------------------------------------
    debug_log.info(f"\n{PRINT_COLORS["green"]}Step 1: Data inventory and setup...{PRINT_COLORS["end"]}")
    ...
    df_IAM_projection = process_IAM_data.read_process_IAM_data(project_dir, scenario, model, file_IAM_model_region_numbers, [varname_SE], years_downscaling)
    ...
    # ----------------------------------------------------------------------------------------------------------
    debug_log.info(f"\n{PRINT_COLORS["green"]}Step 2: Prepare grid and model datasets for harmonisation...{PRINT_COLORS["end"]}")
    ...
    if process_flags["process_SE"] or not se_downscaling_file.is_file():
        ...
        xr_se_downscaling.to_netcdf(se_downscaling_file, mode="w", engine="h5netcdf")
    ...
    # ----------------------------------------------------------------------------------------------------------
    debug_log.info(f"\n{PRINT_COLORS["green"]}Step 3: Calculate regional grid sums for gridded data...{PRINT_COLORS["end"]}")
    ...
    # ----------------------------------------------------------------------------------------------------------
    debug_log.info(f"\n{PRINT_COLORS["green"]}Step 4: Calculate cell-specific correction factors for harmonisation...{PRINT_COLORS["end"]}")
    ...
    # ----------------------------------------------------------------------------------------------------------
    debug_log.info(f"\n{PRINT_COLORS["green"]}Step 5: Applying correction factors to grid data...{PRINT_COLORS["end"]}")
    ...
    t0_save_ = time.time()
    xr_se_harmonised.to_netcdf(se_harmonised_file, mode="w", engine="h5netcdf")
    ...
    # ----------------------------------------------------------------------------------------------------------
    debug_log.info(f"\n{PRINT_COLORS["green"]}Step 6: Performing checks on harmonised data...{PRINT_COLORS["end"]}")
...
def downscale_emissions(scenario:str, model:str="IMAGE", profile:str="default", net_emissions:bool=True):
    ...
    df_population = None
    xr_population = None
    debug_log.info(f"{PRINT_COLORS["green"]}Reading (and processing) population data...{PRINT_COLORS["end"]}")
    if process_flags["read_process_POP"] or pop_file.is_file()==False:
        ...
        xr_population.to_netcdf(pop_file, mode="w", engine="h5netcdf")
    ...
    f_gdp_ppp = None
    xr_gdp_ppp = None
    debug_log.info(f"{PRINT_COLORS["green"]}Reading (and processing) GDP (PPP) data...{PRINT_COLORS["end"]}")
    ...
    if process_flags["read_process_GDP_PPP"] or gdp_ppp_file.is_file()==False:
        ...
        xr_gdp_ppp.to_netcdf(gdp_ppp_file, mode="w", engine="h5netcdf")
        debug_log.info("--------------------------------")
    ...
    f_emissions = None
    xr_emissions = None
    debug_log.info(f"\n{PRINT_COLORS["green"]}Reading (and processing) emissions data...{PRINT_COLORS["end"]}")
    ...
    if process_flags["process_IAM"]:
        debug_log.info(f"\n{PRINT_COLORS["green"]}Reading and processing IAM data...{PRINT_COLORS["end"]}")
    ...
    # 2.1.1 grid
    debug_log.info(f"\n{PRINT_COLORS["green"]}Processing GDP and population data for downscaling...{PRINT_COLORS["end"]}")
    ...
    if process_flags["process_GDP_POP_grid"]:
        ...
        xr_population_processed.to_netcdf(pop_processed_file, mode="w", engine="h5netcdf")
        xr_gdp_ppp_processed.to_netcdf(gdp_ppp_processed_file, mode="w", engine="h5netcdf")
        ...
        xr_gdp_ppp_per_population = process_IPAT_factors.calculate_gdp_per_pop(xr_population_processed, xr_gdp_ppp_processed,
                                                                               ...
                                                                               unit_POP, unit_GDP_PPP)
        xr_gdp_ppp_per_population.to_netcdf(gdp_ppp_per_pop_file, mode="w", engine="h5netcdf")
    ...
    debug_log.info(f"\n{PRINT_COLORS["green"]}Harmonising IAM regions grid with population and GDP grid...{PRINT_COLORS["end"]}")
    ...
    if process_flags["process_GDP_per_POP"]:
        debug_log.info(f"{PRINT_COLORS["green"]}Calculating GDP per capita for IAM data...{PRINT_COLORS["end"]}")
    ...
    debug_log.info(f"\n{PRINT_COLORS["green"]}Processing GDP and emissions data for downscaling...{PRINT_COLORS["end"]}")
    # 2.2.1 process grid
    ...
    df_IAM_EM = process_IAM_data.process_EM_regions_data(df_IAM, years_downscaling, varname_EM, net_emissions, model, debug_log)
    ...
    if process_flags["process_df_EM_per_GDP"]:
        debug_log.info(f"{PRINT_COLORS["green"]}Calculating EM per GDP (PPP) for IAM data...{PRINT_COLORS["end"]}")
    ...
    if process_flags["process_grid_EM_per_GDP"]:
        debug_log.info(f"{PRINT_COLORS["green"]}Calculating CO2 grid emissions...{PRINT_COLORS["end"]}")
        ...
        xr_em_per_gdp_ppp.to_netcdf(em_per_gdp_ppp_file, mode="w", engine="h5netcdf")
        if process_flags["save_tiffs"]:
        ...
        xr_em.to_netcdf(em_unharmonised_file, mode="w", engine="h5netcdf")
    else:
    ...
    print(f"{PRINT_COLORS["green"]}Harmonising grid emissions per region with IAM emissions per region...{PRINT_COLORS["end"]}")
    ...
    xr_em_grid_correction.to_netcdf(em_harmonised_file, mode="w", engine="h5netcdf")
    ...
    debug_log.info(f"\n{PRINT_COLORS["green"]}Downscaling complete. Processed data saved to {dir_processed} and output to {dir_output}.{PRINT_COLORS["end"]}")

    # aggregate to urban, peri-urban, and rural areas per country IAM region
    if process_flags["process_urban_classification"]:
        ds_urban_classification = xr.open_dataset(project_dir / "data/input/DLL/DDL_Class_Sept_2025.tif", engine="rasterio")
        ds_urban_classification = ds_urban_classification.squeeze("band", drop=True)
        #ds_urban_classification = ds_urban_classification.rename({"x": "longitude", "y": "latitude"})
        ds_urban_classification = ds_urban_classification.assign_coords(latitude=xr_em_grid_correction.y, longitude=xr_em_grid_correction.x)
        ds_urban_classification.to_netcdf(dir_processed / f"urban_classification_{model}_{scenario}.nc", mode="w", engine="h5netcdf")
    else:
        ds_urban_classification = xr.open_dataset(dir_processed / f"urban_classification_{model}_{scenario}.nc")

    # df_urban_emissions = process_IPAT_factors.calc_urban_regional_emissions(xr_em_grid_correction, varname_EM, ds_urban_classification, years_downscaling)
    # csv_file_ruban_emissions = dir_processed / f"df_urban_emissions_{model}_{scenario}.csv"
    # df_urban_emissions.to_csv(csv_file_ruban_emissions, sep=";", index=False)

    xr_population.close()
    xr_gdp_ppp.close()
    xr_emissions.close()
    xr_IAM_regions_grid.close()
    xr_population_processed.close()
    xr_gdp_ppp_processed.close()
    xr_gdp_ppp_per_population.close()
    xr_em.close()
...
def plot_results(scenario:str = "ELV-SSP2-CP", model:str="IMAGE", profile:str = "default", net_emissions:bool=True):
    ...
    vars_CO2_excl_ship_av_AFOLU = process_IAM_data.list_emissions_excl_ship_av_AFOLU()
    vars_downscaling = [varname_POP, varname_GDP, *vars_CO2_excl_ship_av_AFOLU]
    years_downscaling = [2020, 2030, 2040, 2050, 2060, 2070, 2080, 2090, 2100]

    df_IAM = process_IAM_data.read_process_IAM_data(project_dir, scenario, model, file_IAM_model_region_numbers, vars_downscaling, years_downscaling)
    df_IAM_POP = df_IAM[df_IAM["variable"]==varname_POP]
    df_IAM_GDP = df_IAM[df_IAM["variable"]==varname_GDP]
    df_IAM_EM = process_IAM_data.process_EM_regions_data(df_IAM, years_downscaling, varname_EM, 2150, model)
    ...
    for y in [2020, 2030, 2050]:
        plot_maps.plot_hist(dir_processed, xr_emissions_proj, varname_EM, "", y)

    plot_maps.plot_boxplot_per_region(project_dir, dir_processed, file_IAM_model_region_numbers,
                                      xr_emissions_proj, varname_EM,
                                      model, scenario, [2020, 2030, 2050, 2100])

    plot_maps.plot_hist_map(dir_processed, xr_population_hist, varname_POP, 2020)
    plot_maps.plot_hist_map(dir_processed, xr_gdp_ppp_hist, varname_GDP, 2020)
    plot_maps.plot_hist_map(dir_processed, xr_emissions_proj, varname_EM, 2020)
    plot_maps.plot_hist_map(dir_processed, xr_emissions_proj, varname_EM, 2030)
    plot_maps.plot_hist_map(dir_processed, xr_emissions_proj, varname_EM, 2050)

    #plot_results_urban_emissions(model, scenario)
...
if __name__ == "__main__":
    ...
    if hasattr(arguments, 'process') and arguments.process is not None:
        ...
        if arguments.process == "copy":
            process_datasets(arguments.ssp_baseline, copy=True)
        else:
            process_datasets(arguments.ssp_baseline, copy=False)
    if hasattr(arguments, 'create_GADM_raster') and arguments.create_GADM_raster is True:
        ...
        create_GADM_region_raster("IMAGE", float(arguments.resolution), True)
    if hasattr(arguments, 'downscale_population') and arguments.downscale_population is True:
        ...
        downscale_SE_data("Population", arguments.scenario, arguments.model, False, arguments.profile)
    if hasattr(arguments, 'downscale_gdp_ppp') and arguments.downscale_gdp_ppp is True:
        if arguments.scenario is None or arguments.profile is None:
            parser.error("--scenario requires a scenario to be specified and/or --profile requires a profile to be specified")
        downscale_SE_data("GDP|PPP", arguments.scenario, arguments.model, True, arguments.profile)
    if hasattr(arguments, 'downscale_emissions') and arguments.downscale_emissions is True:
        ...
        if arguments.emissions not in ["net", "gross"]:
            ...
            net_emissions = False
        downscale_emissions(arguments.scenario, arguments.model, arguments.profile, net_emissions)
    if hasattr(arguments, 'plot') and arguments.plot is True:
        if arguments.scenario is None or arguments.profile is None:
            parser.error("--scenario requires a scenario to be specified and/or --profile requires a profile to be specified")
        plot_results(arguments.scenario, "IMAGE", arguments.profile, True)
    if hasattr(arguments, 'upload') and arguments.upload is True:
        ...
        upload_to_GEE(arguments.scenario, "IMAGE", arguments.profile)
    if hasattr(arguments, 'compare') and arguments.compare is True:
        compare_two_raster_files()
